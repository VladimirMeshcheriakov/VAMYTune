...
void note_change_state(const Uint8 *piano_keys, ADSR *env, Uint8 *past_occ, SDL_AudioDeviceID audio_device_id, ud *data, int key_id, char note)
{
...
void note_state(const Uint8 *piano_keys, Uint8 *past_occ, ADSR *env, SDL_AudioDeviceID audio_device_id, ud *data)
{
    if (piano_keys[22] != past_occ[0])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,22,'s');
    }
    if (piano_keys[8] != past_occ[1])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,8,'e');
    }
    if (piano_keys[7] != past_occ[2])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,7,'d');
    }
    if (piano_keys[21] != past_occ[3])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,21,'r');
    }
    if (piano_keys[9] != past_occ[4])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,9,'f');
    }
    if (piano_keys[10] != past_occ[5])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,10,'g');
    }
    if (piano_keys[28] != past_occ[6])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,28,'y');
    }
    if (piano_keys[11] != past_occ[7])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,11,'h');
    }
    if (piano_keys[24] != past_occ[8])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,24,'u');
    }
    if (piano_keys[13] != past_occ[9])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,13,'j');
    }
    if (piano_keys[12] != past_occ[10])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,12,'i');
    }
    if (piano_keys[14] != past_occ[11])
    {
        // STATE CHANGED
        note_change_state(piano_keys,env,past_occ,audio_device_id,data,14,'k');
    }
...
void update_effects(float *effects,ud *data, ADSR *env, TimeStamp **time_data)
{
